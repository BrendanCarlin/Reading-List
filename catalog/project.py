...
def readingListJSON(readingList_id):
	reading_list = session.query(readingList).filter_by(id = readingList_id).one()
...
@app.route('/reading-lists/<int:readingList_id>/delete/', methods = ['GET', 'POST'])
...
def showItems(list_id):
	...
	if 'username' not in login_session:
		return render_template('publicitems.html', books = books, book_list = book_list, list_id = list_id, creator = creator)
	else:
		return render_template('items.html', books = books, book_list = book_list, list_id = list_id, creator = creator)
...
def newItem(list_id):
	...
	if request.method == 'POST':
		...
		return render_template('new_item.html', list_id = list_id, list = book_list)


# This page will let a user edit a specifc item
@app.route('/reading-lists/<int:list_id>/books/<int:book_id>/edit/', methods = ['GET', 'POST'])
def editItem(list_id, book_id):
	...
	if request.method == 'POST':
		...
		return render_template('edit_item.html', list_id = list_id, book_id = book_id, item = editedBook, list = reading_list)


# This page will let a user delete a specific item
@app.route('/reading-lists/<int:list_id>/books/<int:book_id>/delete/', methods = ['GET', 'POST'])
def deleteItem(list_id, book_id):
	...
	if request.method == 'POST':
		...
		return render_template('delete_item.html', book = bookToDelete, list_id = list_id, list = reading_list)
